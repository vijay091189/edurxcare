...
@section('title', 'Edurxcare - Allergies')
@section('content')
...
<div class="container-fluid">
            ...
            <div class="row">
               ...
               <div class="col-sm-12">
                  ...
                  <div class="card">
                     <div class="col-sm-12">
                        <a class="btn btn-primary add_new_btn" onclick="modalpopup();">Add Allergy</a>
                     </div>
                              ...
                              <table class="list_table">
                                 ...
                                 <tbody>
                                    ...
                                    @foreach($categories as $category)
                                       <tr>
                                          <td>{{ $key }}</td>
                                          <td>{{ $category->allergy_name }}</td>
                                          <td>
                                             <a href="#!" onclick="edit_category('{{ $category->id }}','{{ $category->allergy_name }}');">
   ...
   <div class="modal-body">
      <form action="#" method="post" name="categoryform" id="categoryform">
         ...
         <div class="form-group row">
            <div class="col-sm-3">
               <label>Allergy <span class="text-danger">*</span></label>
            </div>
            <div class="col-sm-6">
...
function savecategory(){
   ...
   var post_url = "{{URL::to('/saveAllergies')}}";
   ...
   $.ajax({
      ...
      success : function(result){
         if(result=='exist'){
            alert("Record already exists with the Allergy name '"+category_name+"'");
            return false;
         }	
         if(category_id==''){
...
function deleteCategory(unique_id,status){
   var post_url = "{{URL::to('/deleteAllergies')}}?category_id="+unique_id+"&status="+status;
   ...
   if(status=='0'){
      var message = 'Are you sure you want to make this allergy Inactive?';
      var status_text = 'Inactive';
   } else {
      var message = 'Are you sure you want to make this allergy Active?';
   ...
   if (confirm(message)) {
      $.ajax({
         url : post_url,
         success : function(result){	
            alert("Allergy made "+status_text+" successfully");
            //location.reload();
            if(status=='0'){
